...
7. Prepare the next development version.
   ...
   - Add or reset the top changelog section according to the existing changelog format.
...
3. Ensure the changelog is fully updated using an available changelog skill or explicit user direction.
...
8. Checkout the development branch and merge main with `--no-ff --no-edit`.
